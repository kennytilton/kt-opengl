...
(defpackage :gears
  (:use :common-lisp :clos :utils-kt :cells :celtk :cello))
...
(defmodel gears-demo (window)
  ((gear-ct :cell t :initform (c-in 1) :accessor gear-ct :initarg :gear-ct)
   (scale   :cell t :initform (c-in 1) :accessor scale :initarg :scale))
  (:default-initargs
     ...
     :kids (c? (the-kids
                 (mk-stack (:packing (c?pack-self "-side left -fill both"))
                   ...
                   (make-instance 'gears
                     ...
                     :timer-interval
                       (c? (let ((n$ (md-value (fm-other :vtime))))
